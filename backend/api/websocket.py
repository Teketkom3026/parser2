...
async def task_progress(websocket: WebSocket, task_id: str):
    ...
    tm.register_progress_callback(task_id, send_progress)
    try:
        ...
        if task:
            await websocket.send_json({
                "task_id": task_id,
                "status": task["status"],
                "processed": task["processed_urls"],
                "total": task["total_urls"],
                "found_contacts": task["found_contacts"],
                "errors": task["errors_count"],
                "current_url": "",
                "message": f"Подключено. Статус: {task['status']}",
            })

        # Держим соединение открытым, ждём сообщения или отключения
        while True:
            try:
