"""Генерация итогового Excel-файла."""
...
HEADERS = [
    ...
    "ФИО сотрудника",           # E
    "Должность (как на сайте)", # F
    "Должность (нормализованная)", # G
    "Категория роли",           # H
    "Личный email",             # I
    "Личный телефон",           # J
    "ИНН",                      # K
...
def generate_excel(contacts: list[dict], output_path: str) -> str:
    ...
    # ── Ширины колонок ──
    col_widths = [25, 20, 25, 20, 28, 30, 30, 20, 28, 20, 14, 12, 30, 40, 8, 14, 10, 25]
    ...
    # ── Автофильтр ──
    ws.auto_filter.ref = f"A1:R{len(contacts) + 1}"

    # ── Лист-сводка ──
    ws2 = wb.create_sheet("Сводка")
    ws2.cell(row=1, column=1, value="Метрика").font = Font(bold=True)
    ws2.cell(row=1, column=2, value="Значение").font = Font(bold=True)

    total = len(contacts)
    with_name = sum(1 for c in contacts if c.get("person_name"))
    with_email = sum(1 for c in contacts if c.get("person_email"))
    with_position = sum(1 for c in contacts if c.get("position_norm"))
    unique_companies = len(set(c.get("site_url", "") for c in contacts))

    summary_data = [
        ("Всего записей", total),
        ("С ФИО сотрудника", with_name),
        ("С личным email", with_email),
        ("С должностью", with_position),
    ...
    wb.save(output_path)
    logger.info("excel_generated", path=output_path, rows=total)
    return output_path
