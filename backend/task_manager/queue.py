"""Менеджер задач на asyncio.Queue — оркестрация парсинга."""
import asyncio
import json
import random
import time
...
class TaskManager:
    ...
    async def _process_task(self, task_id: str) -> None:
        ...
        all_contacts = []
        try:
            pending_sites = await self.db.get_pending_sites(task_id)
            for site_row in pending_sites:
                site_id = site_row["id"]
                url = site_row["url"]
                ...
                try:
                    ...
                    logger.error("site_processing_error", url=url, error=error_str)
                ...
                # Адаптивная пауза между сайтами
            ...
            logger.info("task_completed", task_id=task_id, summary=summary.to_dict())
        except Exception as e:
    ...
    async def _process_site(
        ...
        """Обработать один сайт. Возвращает список контактов."""
        ...
        if not await is_allowed(url):
            ...
            return [{
                ...
                "role_category": "",
                "status": "Ошибка",
                "comment": "robots.txt запрещает доступ",
            }]
        ...
        # Regex-данные с главной
        regex_data = extract_all_regex(main_html, clean_html(main_html))
        company_emails = regex_data["emails"][:3]
        company_phones = regex_data["phones"][:3]
        inn_list = regex_data["inn"]
        kpp_list = regex_data["kpp"]

        # Язык
        page_text = clean_html(main_html)
        lang = detect_language(page_text)

        # Поиск страниц с руководством/командой
        ...
        # Извлечение контактов со всех найденных страниц
        all_persons = []
        for page_url in pages_to_parse:
            try:
                ...
                # DOM-извлечение персон
                person_blocks = extract_person_blocks(html)

                # Regex-данные со страницы
                page_regex = extract_all_regex(html, clean_html(html))

                # Обновляем общие email/телефон/ИНН/КПП
                for e in page_regex["emails"]:
                    if e not in company_emails:
                        company_emails.append(e)
                for p in page_regex["phones"]:
                    if p not in company_phones:
                        company_phones.append(p)
                ...
                for pb in person_blocks:
                    person_dict = {
                        ...
                        "person_name": pb.name or "ФИО не найдено",
                        "position_raw": pb.position,
                        "position_norm": normalize_position(pb.position),
                        "role_category": classify_role(pb.position),
                        "person_email": pb.email,
                        "person_phone": pb.phone,
                        ...
                        "status": "OK" if pb.name else "Частично",
                        "comment": "" if pb.name else "ФИО не найдено",
                        "scan_date": datetime.now().strftime("%Y-%m-%d"),
                    }
                    all_persons.append(person_dict)
            except Exception as e:
                logger.warning("page_parse_error", page_url=page_url, error=str(e))
                continue

        # Фильтрация по должностям (Режим 1)
        if mode == "mode_1" and target_positions:
            all_persons = filter_by_positions(all_persons, target_positions)

        # Если ничего не нашли — создаём запись об ошибке
        if not all_persons:
            all_persons.append({
                "site_url": url,
                "page_url": url,
                ...
                "extraction_method": "regex",
                "status": "Частично",
                "comment": "Контакты сотрудников не найдены",
                "scan_date": datetime.now().strftime("%Y-%m-%d"),
            })
